"""
Endpoint for CRUD operations on Stack Infrastructure Templates
"""
...
from core.manager.stackl_manager import get_document_manager  # pylint disable=
...
def get_stack_infrastructure_template_by_name(
    ...
    logger.info(
        f"GET request for type 'stack_infrastructure_template' and document '{name}'"
    )
...
def delete_stack_infrastructure_template(
        name: str,
        document_manager: DocumentManager = Depends(get_document_manager)):
    """Deleta a stack infrastructure template by name"""
