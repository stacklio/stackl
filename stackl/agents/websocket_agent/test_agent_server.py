...
def get_config_key(key):
    ...
    print("[Agent] get_config_key. Key: '{0}'. Value: '{1}'".format(
        key, value))
    if value:
        return value
    else:
...
class Agent_Server_Test:
    ...
    def run_worker_websocket(self):
        ...
        print(
            "[Agent_Server_Test] run_worker_websocket started for agent_url '{}'"
            .format(agent_url))
        agent_worker_loop = asyncio.new_event_loop()
        asyncio.set_event_loop(agent_worker_loop)
        self.agent_websocket = websockets.serve(self.worker_connection_handler,
                                                host=agent_url["host"],
                                                port=agent_url["port"])
    ...
    def get_agent_url(self):
        ...
        print(
            "[Agent_Server_Test] Returning agent url '{}'.".format(agent_url))
        return agent_url

    async def worker_connection_handler(self, websocket, path):
        print("[Agent_Server_Test] worker_connection_handler activated.")
        worker_recv_task = asyncio.create_task(
            self.worker_recv_handler(websocket, path))
        worker_send_task = asyncio.create_task(
            self.worker_send_handler(websocket, path))
    ...
    async def worker_recv_handler(self, websocket, path):
        ...
        async for message in websocket:
            print(
                "[Agent_Server_Test] worker_recv_handler. Received message from Worker: '{}'"
                .format(message))
    ...
    async def worker_send_handler(self, websocket, path):
        ...
        while True:
            ...
            print(
                "[Agent_Server_Test] worker_send_handler. Sending message '{}'"
                .format(message))
    ...
    async def producer(self):
        ...
        print(
            "[Agent_Server_Test] producer flag set to true. Activating producer."
        )
    ...
    def test_periodic(self):
        ...
        while True:
            ...
            print("[Agent_Server_Test] test_periodic run '{}', every 30sec.".
                  format(count))
...
if __name__ == "__main__":
    try:
        ...
        print(
            "[Agent_Server_Test] !!!ERROR!!! EXCEPTION OCCURED IN AGENT TOP LEVEL:'{}'. Exit."
            .format(e))
