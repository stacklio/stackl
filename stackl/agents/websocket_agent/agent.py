...
def get_config_key(key):
    ...
    logger.info("[Agent] get_config_key. Key: '{0}'. Value: '{1}'".format(
        key, value))
    if value:
        if re.match(r"\[.*?\]", value):
...
class Agent:
    def __init__(self, stackl_url):
        ...
        worker_websocket_thread = threading.Thread(
            target=self.run_worker_websocket)
        worker_websocket_thread.start()

        stackl_websocket_thread = threading.Thread(
            target=self.run_stackl_websocket)
    ...
    def run_worker_websocket(self):
        ...
        logger.info(
            "[Agent] run_worker_websocket. Starting new async loop for agent_url '{}'"
            .format(agent_url))
        ...
        self.worker_websocket = websockets.serve(
            self.worker_connection_handler,
            host="0.0.0.0",
            port=agent_url["port"]
        )  # Has to be 0.0.0.0 for containers: see https://serverfault.com/questions/769578/curl-56-recv-failure-connection-reset-by-peer-when-hitting-docker-container
    ...
    async def worker_connection_handler(self, websocket, path):
        ...
        worker_recv_task = asyncio.ensure_future(self.worker_recv_handler(
            websocket, path),
                                                 loop=loop)
        worker_send_task = asyncio.ensure_future(self.worker_send_handler(
            websocket, path),
                                                 loop=loop)
        ...
        if worker_recv_task in done:
            logger.info(
                "[Agent] worker_connection_handler. worker_recv_task {0} in done."
                .format(worker_recv_task))
        if worker_send_task in done:
            logger.info(
                "[Agent] worker_connection_handler. worker_send_task {0} in done."
                .format(worker_send_task))
        for task in pending:
            logger.info(
                "[Agent] worker_connection_handler. Task {0} in pending. Cancelling"
                .format(task))
            task.cancel()
    ...
    async def worker_recv_handler(self, websocket, path):
        ...
        while True:  # NOTE: async for message in websocket is Python 3.6+
            ...
            logger.info(
                "[Agent] worker_recv_handler. Received message from Worker: '{}'"
                .format(message))

            msg_obj = json.loads(message)
            msg_obj.update({
                "stack_instance_status":
                "Stack Instance Received by the Agent!"
            })

            logger.info(
                "[Agent] Replying to worker with updated msg. Message: '{}'".
                format(msg_obj))
    ...
    async def worker_send_handler(self, websocket, path):
        ...
        while True:
            ...
            logger.info(
                "[Agent] worker_send_handler. Sending message: '{}'".format(
                    message))
    ...
    async def processor(self):
        ...
        if isinstance(msg_obj, str):
            logger.info(
                "[Agent] processor. Message is str. Message: '{}'".format(
                    msg_obj))
        elif isinstance(msg_obj, dict):
            if msg_obj.get("automation_handler", False):
                logger.info(
                    "[Agent]  processor. Handing to automation_handler")
                return "____MSG_AUTOMATION_ECHO____" + msg_obj
            else:
                msg_obj.update({
                    "stack_instance_status":
                    "Stack Instance Received and Processed by the Agent!"
                })
                logger.info(
                    "[Agent] processor. Message is  dictionary. Updating with success. Message: '{}'"
                    .format(msg_obj))
                return msg_obj
        else:
            logger.info(
                "[Agent] Fallback processor, just sleeping 5s and returning the reverse message"
            )
    ...
    def run_stackl_websocket(self):
        ...
        while True:
            try:
                stackl_websocket_loop.run_until_complete(
                    self.connect_to_stackl())
                stackl_websocket_loop.run_until_complete(
                    self.stackl_recv_handler(self.ws))
            except Exception as e:
                logger.info(
                    "[Agent] Error! Did the connection die? Exception: '{}'. Restarting STACKL connection."
                    .format(e))
    ...
    async def connect_to_stackl(self):
        while True:
            try:
                logger.info(
                    "[Agent] connect_to_stackl. Trying to connect to '{}'".
                    format(self.stackl_url))
                self.ws = await websockets.client.connect(self.stackl_url)
                await self.ws.send(
                    json.dumps(
                        "-----> Agent '{0}' is connecting to Stackl. The agent_connect_info is:"
                        .format(self)))
                await self.ws.send(json.dumps(self.get_agent_connect_info())
                                   )  # need to send this seperately, as dict
                logger.info("[Agent] Connection succeeded!")
                break
            except Exception as e:
                logger.info(
                    "[Agent] connect_to_stackl. Connecting failed. Error: '{}'"
                    .format(e))
                logger.info(
                    "[Agent] connect_to_stackl. Trying again in 15 seconds.")
    ...
    def test_periodic(self):
        ...
        while True:
            ...
            logger.info(
                "[Agent] test_periodic run '{}', every 30sec.".format(count))
    ...
    async def stackl_recv_handler(self, websocket):
        async for message in websocket:
            logger.info(
                "[Agent] stackl_recv_handler. Received message '{}' ".format(
                    message))
    ...
    async def stackl_send_handler(self, websocket):
        while True:
            ...
            logger.info(
                "[Agent] stackl_send_handler. Sending message '{}' ".format(
                    message))
            await websocket.send(json.dumps(message))
    ...
    def get_agent_connect_info(self):
        agent_url = {
            "is_agent_url": True,
            "host": get_config_key("HOST_MACHINE"),
            "port": 8889,
...
if __name__ == "__main__":
    try:
        ...
        logger.error(
            "[Agent] Exception occured at top level:'{}'. Exit.".format(e))
