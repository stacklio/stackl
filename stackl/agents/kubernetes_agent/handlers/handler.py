...
class Handler:
    ...
    def create_variables_cm(self, name, namespace, stack_instance, service,
                            variables_config):
    ...
    def create_job_object(self,
        ...
        body = self.create_kubernetes_job(args, cmd, container_image,
                                          container_name, name, namespace,
                                          pull_secrets, service_account)
        return body

    def add_vault_agent_init_container(self, body: client.V1Job, vault_config):
        vault_agent_env_list = [
            client.V1EnvVar(name="VAULT_ADDR", value=vault_config["address"])
        ]

        templated_vault_config = vault_agent_config % (
            vault_config["contents"], vault_config["destination"])
        print(templated_vault_config)
        cm = self.create_agent_cm(body.metadata.name + "-vault-agent",
                                  body.metadata.namespace,
                                  templated_vault_config)
        volumes = []
    ...
    def create_kubernetes_job(self, args, cmd, container_image, container_name,
                              name, namespace, pull_secrets, service_account):
        ...
        volume_mounts = []
        volume_mount = client.V1VolumeMount(name="stackl-variables",
                                            mount_path="/etc/stackl-variables")
    ...
    def delete_job_object(self,
        ...
        template.template = client.V1PodTemplateSpec()
        env_list = [
            client.V1EnvVar(name="TF_VAR_stackl_stack_instance",
                            value=stack_instance),
            client.V1EnvVar(name="TF_VAR_stackl_service", value=service),
            client.V1EnvVar(name="TF_VAR_stackl_host",
                            value=os.environ['stackl_host'])
        ]
        container = client.V1Container(
            ...
            command=["/bin/sh", "-c"],
            args=[
                "terraform init -backend-config=address=http://" +
                os.environ['stackl_host'] + "/terraform/" + stack_instance +
    ...
    def handle(self, invocation, action):
        ...
        print("create cm")
        config_map = self.create_variables_cm(name, stackl_namespace,
        ...
        if action == "create" or action == "update":
            body = self.create_job_object(
                ...
                cmd=tool_config["create_command"],
            ...
            if 'vault' in tool_config["secrets"]:
                ...
                # add init container
                body, vault_config_map = self.add_vault_agent_init_container(
                    body, tool_config["secrets"]["vault"])
                try:
                    api_response = self.api_instance_core.create_namespaced_config_map(
                        stackl_namespace, vault_config_map)
                except ApiException as e:
                    print(
                        "Exception when calling V1Api->create_namespaced_configmap: %s\n"
                        % e)
        else:
