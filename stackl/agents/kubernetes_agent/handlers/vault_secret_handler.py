...
template {
  contents = <<EOH
  "{{- with secret "secret/data/infra/test?version=1" }}{{ .Data.data | %s }}{{ end }}"
  EOH
...
class VaultSecretHandler(SecretHandler):
    ...
    def handle_secret(self, kubernetes_body: client.V1Job):
        ...
        vault_agent_env_list = [client.V1EnvVar(name="VAULT_ADDR", value="http://10.11.12.7:8200")]

        templated_vault_config = self.format_template()
        print(templated_vault_config)
        cm = _create_agent_cm(body.metadata.name + "-vault-agent", body.metadata.namespace, templated_vault_config)
        volumes = []
    ...
    def get_init_container(self):
        return client.V1Container(name="vault-agent",
                                  image="vault",
                                  args=["agent",
