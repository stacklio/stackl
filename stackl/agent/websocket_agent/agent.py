...
class Agent:
    ...
    def run_worker_websocket(self):
        ...
        self.worker_websocket = websockets.serve(self.worker_connection_handler, host = "0.0.0.0", port = agent_url["port"])  #Has to be 0.0.0.0 for containers: see https://serverfault.com/questions/769578/curl-56-recv-failure-connection-reset-by-peer-when-hitting-docker-container
    ...
    async def processor(self):
        ...
        if isinstance(msg_obj, str):
            ...
            if msg_obj.get("automation_handler", False):
                ...
                logger.log("[Agent] processor. Message is  dictionary. Updating with success. Message: '{}'".format(msg_obj))
                return msg_obj
        else:
    ...
    def run_stackl_websocket(self):
        ...
        while True:
            try:
                ...
                logger.log("[Agent] Error! Did the connection die? Exception: '{}'. Restarting STACKL connection.".format(e))
    ...
    async def connect_to_stackl(self):
        while True:
            try:
                ...
                await self.ws.send(json.dumps("-----> Agent '{0}' is connecting to Stackl. The agent_connect_info is:".format(self)))
    ...
    def get_agent_connect_info(self):
        agent_url = { "is_agent_url": True, "host" : get_config_key("HOST_MACHINE"), "port" : 8889, "tags" : get_config_key("TAGS")}
