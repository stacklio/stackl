...
(function(factory) {
  ...
  $.fn.orgchart = function(options) {
    ...
    switch (options) {
      ...
      case 'getHierarchy': {
        if (!$(this).find('.node')[1].id) {
          return 'Error: Nodes of orghcart to be exported must have id attribute!';
        }
        var objects = new Array()
        return getHierarchy.apply(this, [$(this), objects]);
      }
  ...
  function createNode(nodeData, level, opts) {
    ...
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' +
        ' name="' + nodeData[opts.nodeTitle] + '"' + ' type="' + nodeData[opts.nodeType] + '"': '') + '>')
      .addClass('node' + (level >= opts.depth ? ' slide-up' : ''))
      .append('<div class="title ' + nodeData[opts.nodeType] + '" >' + nodeData[opts.nodeTitle] + '</div>')
    ...
    if(nodeData[opts.nodeType] == 'common') {
      $nodeDiv.children('.title').prepend('<i class="glyphicon glyphicon-asterisk symbol"></i>');
    };

    if(nodeData[opts.nodeType] == 'environment') {
      $nodeDiv.children('.title').prepend('<i class="glyphicon glyphicon-tree-deciduous symbol"></i>');
    };

    if(nodeData[opts.nodeType] == 'location') {
      $nodeDiv.children('.title').prepend('<i class="glyphicon glyphicon-globe symbol"></i>');
    };

    if(nodeData[opts.nodeType] == 'zone') {
      $nodeDiv.children('.title').prepend('<i class="glyphicon glyphicon-flash symbol"></i>');
    };
    ...
    if (opts.draggable) {
      ...
      .on('drop', function(event) {
        ...
        bootbox.dialog({
          ...
          message: '<div>You wish to copy or move node <span style="font-weight:bold">' + $dragged.attr('name') +
          '</span> to parent <span style="font-weight:bold">' + $dropZone.attr('name') + '</span></div>',
          buttons: {
            move: {
              label: "Move",
              className: "btn nub-button-move",
              callback: function() {
                var types = validateType($dropZone[0].id);
                if (jQuery.inArray($dragged.attr('type'), types ) > -1){
                  ...
                  moveStorageNode($dragged[0].id, $dragged.attr('type'), $dropZone[0].id);
                } else {
                  $.alert("The node: " + $dragged.attr('name') + "  cannot be moved to new parent " + $dropZone.attr('name'), {
                    autoClose: true,
                    closeTime: 5000,
                    type: 'danger'
                  });
            ...
            copy: {
              label: "Copy",
              className: "btn  nub-button-copy",
              callback: function() {
                var guid = 'id-' + Math.random().toString(36).substr(2, 16);
                var types = validateType($dropZone[0].id);
                if (jQuery.inArray($dragged.attr('type'), types ) > -1){
                  var nodeVals = [];
                  var validVal = $dragged[0].id;
                  if (validVal.length) {
                    nodeVals.push(validVal);
                  }

                  var hasChild = $dropZone.parent().attr('colspan') > 0 ? true : false;
                  if (!hasChild) {
                    var rel = nodeVals.length > 1 ? '110' : '100';
                    $('#chart-container').orgchart('addChildren', $dropZone, {
                      'children': nodeVals.map(function(item) {
                        return { 'guid': guid, 'name': $dragged.attr('name'), 'type': $dragged.attr('type'), 'relationship': rel };
                      })
                    }, $.extend({}, $('#chart-container').data('orgchart').options, { depth: 0 }));
                  } else {
                    $('#chart-container').orgchart('addSiblings', $dropZone, $dropZone.closest('tr').siblings('.nodes').find('.node:first'),
                        {
                          'siblings': nodeVals.map(function(item) {
                            return { 'guid': guid, 'name': $dragged.attr('name'), 'type': $dragged.attr('type'), 'relationship': '110' };
                          })
                        });
                  }
                  copyStorageNode($dragged[0].id, $dragged.attr('type'), $dropZone[0].id, guid);
                } else {
                  $.alert("The node: " + $dragged.attr('name') + "  cannot be moved to new parent " + $dropZone.attr('name'), {
                    autoClose: true,
                    closeTime: 5000,
        ...
        moveNodeInLocalStorage($dragged, $dropZone, $dragZone)
*/
      });
    }
  ...
  function buildHierarchy ($appendTo, nodeData, level, opts, callback) {
    ...
    if (Object.keys(nodeData).length > 1) { // if nodeData has nested structure
      $table = $('<table style="border-collapse: initial;">');
  ...
  function addSiblings($parentNode, $node, data, opts) {
