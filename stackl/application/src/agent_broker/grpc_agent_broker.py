...
class GrpcAgentBroker(AgentBroker):
    ...
    def create_change_obj(self, stack_instance, action, document_manager):
        self.logger.log("[GrpcAgentBroker] create_change_obj. For stack_instance '{0}' and action '{1}'".format(stack_instance, action))
        ...
        for service in stack_instance.services:
            ...
            for fr in service_doc["functional_requirements"]:
                ...
                self.logger.log("[GrpcAgentBroker] create_change_obj. Retrieved fr '{0}' from service_doc '{1}''".format(fr_doc, service_doc))
                invoc = change_obj.invocation.add()
                invoc.functional_requirement = fr
                invoc.image = fr_doc['invocation']['image']
                self.logger.log("[GrpcAgentBroker] create_change_obj. service debug '{0}'".format(stack_instance.services[service_name]))
                ...
                self.logger.log("[GrpcAgentBroker] create_change_obj. Added fr '{0}' and invoc '{1}' to change_obj '{2}'"
