...
class TaskBroker(ABC):
    ...
    def remove_task_from_queue(self, task):
        ...
        if task.topic == "result":
            self.logger.log("[{2}] Task is ResultTask, removing the source_task '{0}' from task_queue '{1}'".format(
                task.source_task, task_queue, self.logger.name))
        ...
        for i in range(len(task_queue)):
            if task_queue[i]["id"] == id_task_to_remove:
                task_queue.pop(i)
                self.logger.log(
                    "[{1}] Task with id '{0}' succesfully removed !".format(id_task_to_remove, self.logger.name))
                return
        self.logger.log(
    ...
    def task_queue_monitor(self, task_signal):
        try:
            while True:
                ...
                self.logger.log(
                    "[{2}] Monitor_threads. len task_queue is '{0}'. Waiting until task added or earliest_task_timer '{1}'".format(
                        len(task_queue), earliest_task_timer["wait_time"], self.logger.name))

                task_signal.wait(timeout=earliest_task_timer["wait_time"])

                self.logger.log(
                    "[{0}] Wait time expired or new task added. Checking if any tasks expired".format(self.logger.name))
                earliest_task_timer = self._determine_wait_time()
                if earliest_task_timer["wait_time"] < 0:
                    self.logger.log("[{0}] Wait time expired. Checking if the waiting task is still in queue".format(
                        self.logger.name))
            ...
            self.logger.error(
                "[{0}] Monitor threads encountered an issue. Exception: '{1}'".format(self.logger.name, e))
    ...
    def _determine_wait_time(self):
        ...
        self.logger.log("[{1}] determine_wait_time determined earliest_task_timer: '{0}'".format(earliest_task_timer,
                                                                                                 self.logger.name))
