...
stack_instance_name_field = api.model('stack_instance_name_field', {
    'stack_instance_name': fields.String(required=True, description='Name for the stack instance', example="web_example_stack1")
})
stack_instance_invoke_field = api.model('stack_instance_invoke_field', {
                    'parameters': fields.Raw(required=True, description='Input parameters used for creating the stack instace', example={"key":"value"}),
                    'infrastructure_template_name': fields.String(required=True, description='The stack infrastructure template to use for the stack instance', example="stack_infrastructure_template_demo_example"),
                    'application_template_name': fields.String(required=True, description='The stack application template to use for the new stack instance', example="stack_application_template_web_example"),
                    'stack_instance_name': fields.String(required=False, description='Name for the new stack instance', example="web_example_stack1")
})
...
class StackInstancesName(StacklApiResource):
    def get(self, stack_instance_name="Example_Instance1"):
        ...
        logger.log("[StackInstancesName GET] Getting document for stack instance " + stack_instance_name + ' using manager')
        stack_instance = document_manager.get_document(type = "stack_instance", document_name = stack_instance_name)
        if not stack_instance:
            return {'return_code': StatusCode.NOT_FOUND, 'message': 'Stack instance '+ str(stack_instance_name) +' not found'},StatusCode.NOT_FOUND
...
class StackInstances(StacklApiResource):
    ...
    def post(self):
        ...
        if not json_data:
            return {'return_code': StatusCode.BAD_REQUEST, 'message': 'Bad request: JSON data is mandatory for this POST request'},StatusCode.BAD_REQUEST
        if 'infrastructure_template_name' not in json_data:
            return {'return_code': StatusCode.BAD_REQUEST, 'message': 'Bad request: infrastructure_template_name not found in request data'},StatusCode.BAD_REQUEST
        if 'application_template_name' not in json_data:
            return {'return_code': StatusCode.BAD_REQUEST, 'message': 'Bad request: application_template_name not found in request data'},StatusCode.BAD_REQUEST
        ...
        infr_template_exists = document_manager.get_document(type ="stack_infrastructure_template", document_name = infr_template_name)
        ...
        if not infr_template_exists:
            return {'return_code': StatusCode.NOT_FOUND, 'message': 'SIT with name '+ str(infr_template_exists) +' does not exist'},StatusCode.NOT_FOUND
        application_template_name = document_manager.get_document(type = 'stack_application_template', document_name = application_template_name)
        logger.log("[StackInstances POST] application_template_name exists (should be the case): "+ str(application_template_name))
        if not application_template_name:
            return {'return_code': StatusCode.NOT_FOUND, 'message': 'SAT with name '+ str(application_template_name) +' does not exist'},StatusCode.NOT_FOUND
        ...
        if stack_instance_exists:
            return {'return_code': StatusCode.CONFLICT, 'message': 'Stack instance '+ str(stack_instance_name) +' already exists'},StatusCode.CONFLICT
    ...
    def put(self):
        ...
        if not infr_template_exists:
            return {'return_code': StatusCode.NOT_FOUND, 'message': 'SIT with name '+ str(infr_template_exists) +' does not exist'},StatusCode.NOT_FOUND
        app_template_name = document_manager.get_document(type="stack_application_template", document_name=app_template_name)
        logger.log("[StackInstances PUT] app_template_name exists (should be the case): "+ str(app_template_name))
        if not app_template_name:
            return {'return_code': StatusCode.NOT_FOUND, 'message': 'SAT with name '+ str(app_template_name) +' does not exist'},StatusCode.NOT_FOUND
...
class StackInstanceStateShort(StacklApiResource):
    def get(self,stack_instance_name):
        ...
        if not stack_instance_exists:
            return {'return_code': StatusCode.NOT_FOUND, 'message': 'Stack instance ' + str(stack_instance_name) + ' not found'},StatusCode.NOT_FOUND
        stack_instance_obj_state = stack_manager.get_stack_object_state('types', "stack_instance", stack_instance_name, 'short')
...
class StackInstanceStateLong(StacklApiResource):
    def get(self,stack_instance_name):
        ...
        if not stack_instance_exists:
            return {'return_code': StatusCode.NOT_FOUND, 'message': 'Stack instance ' + str(stack_instance_name) + ' not found'},StatusCode.NOT_FOUND
        stack_instance_obj_state = stack_manager.get_stack_object_state('types', "stack_instance", stack_instance_name, 'long')
        return stack_instance_obj_state
