...
class AuthenticationManager(Manager):
    ...
    def get_clients(self,tenant,serial=None,client_role=None):
        ...
        if serial:
            serial = str(serial)
        auth_doc = self.document_manager.get_document('authentication', category = 'authentication', subcategory = view_doc, document_name = serial)
        if auth_doc:
    ...
    def create_client(self, tenant, client_data):
        if 'role' in client_data and client_data['role'] in ['administrator', 'automation-endpoint', 'agent']:
            ...
            self.logger.info("[AuthenticationManager] Trying to create new client for tenant "+ tenant + " with data "+ str(client_data))
            self.document_manager.write_document('authentication', document_name = client_data['serial'], subcategory = 'client', file = client_data,description=client_data['description'], category ='authentication')
        else:
            raise Exception('[AuthenticationManager] Unsupported role! Must be administrator, automation-endpoint or agent!')


    def delete_client(self, tenant, serial):
        auth_doc = self.document_manager.get_document('authentication', category = 'authentication', subcategory = 'client', document_name = str(serial))
        if auth_doc:
            if ('tenant' not in auth_doc) or ('tenant' in auth_doc and tenant == auth_doc['tenant']):
                ...
                self.document_manager.remove_document( 'authentication', document_name = serial, subcategory = 'client', category='authentication')
            else:
