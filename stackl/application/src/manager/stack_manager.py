...
class StackManager(Manager):
    ...
    def __init__(self, manager_factory):
        ...
        self.logger = Logger("Stack_Manager")
    ...
    def handle_task(self, task):
        self.logger.log("[StackManager] handling subtasks in task {0}".format(task))
        try:
            stack_instance = None
            for subtask in task["subtasks"]:
                self.logger.log("[StackManager] handling subtask '{0}'".format(subtask))
                if subtask == "CREATE":
                ...
                if status_code in [StatusCode.OK, StatusCode.CREATED, StatusCode.ACCEPTED]:
                    ...
                    self.logger.log("[StackManager] Processing subtask succeeded. Sending to agent with connect_info '{0}' the stack_instance '{1}'".format(agent_connect_info, change_obj))
                    ...
                    self.logger.log("[StackManager] Sent to agent. Result '{0}'".format(result))
                else:
                    raise Exception("[StackManager] Processing subtask failed. Status_code '{0}'".format(status_code))

            self.logger.log("[StackManager] Succesfully handled task_attr. Notifying task broker.")
    ...
    def get_stack_object_state(self, category, subcategory, stack_object_id, state_type='short'): #TODO Kept but not updated since we will need to have some introspection capability later on
        ...
        if state_type == 'long':
            stack_instance_obj = self.document_manager.get_document(category = 'stacks', subcategory ='instances', document_name = stack_object_id)
            return resources
        else:
            state_list = []
            state = 'Done'
            for resource in resources:
                for resource_element in resources[resource]:
                    if resources[resource][resource_element]['state'] != 'done' and resources[resource][resource_element]['state'] != 'failed' and resources[resource][resource_element]['state'] != 'interrupted':
                        return 'In progress'
    ...
    def get_stack_instance_resource_state(self, stack_instance_id, resource_name): #TODO Kept but not updated since we will need to have some introspection capability later on
        stack_instance_obj = self.document_manager.get_document(category = 'stacks', subcategory ='instances', document_name = stack_instance_id)
        if not stack_instance_obj:
