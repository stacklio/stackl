...
class CouchDBStore(DataStore):
    ...
    def get(self, tenant="nubera",  **keys):
        ...
        if 'error' in result_content:
            response = self._create_store_response(400, "Error: {0}. Reason: {1}".format(result_content['error'], result_content["reason"]))
            ...
            response = self._create_store_response(result.status_code, result.reason, result_content['rows'][0]['value'])
        elif 'rows' in result_content and len(result_content['rows']) > 1:
            return_obj = []
            ...
            self.logger.log("[CouchDBStore] Something odd happened. Result Status Code: {0}. Content: {1}".format(result.status_code, result_content))
    ...
    def put(self, file, tenant="nubera"):
        document_key = tenant
        self.logger.log("[CouchDBStore] Requests.put on '{0}' with file {1}".format(self.datastore_url + document_key, str(file)))
    ...
    def get_versions(self, tenant="nubera", **keys):
        ...
        if  keys.get("raw", False):
            ...
            for i in list(reversed(range(int(revision_amount)))):
                ...
                if 'reason' in result and (result['reason'] == 'missing') and  'error' in result and (result['error'] == 'not_found'):
                    self.logger.info("[CouchDBStore] get_versions. revision not present anymore... break")
                    break
                result_array.append(result)
            response = self._create_store_response(request_result.status_code, request_result.reason, {"result": result_array})
    ...
    def get_version_x(self, tenant="nubera", **keys):
        ...
        response = self._create_store_response(initial_store_response.status_code, initial_store_response.reason, {"result": document_version_x})
    ...
    def get_categories(self, tenant="nubera"):
        ...
        try:
            ...
            self.logger.error("[CouchDBStore] Exception occured in get_categories: " + str(e) + ". Returning empty StoreResponse") 
    ...
    def get_subcategories_for_category(self, category, tenant="nubera"):
        ...
        self.logger.log("[CouchDBStore] get_subcategories_for_category. Searching for subcategories on key: " + document_key)
        try:
            result = requests.get(self.datastore_url + document_key, verify='/certs/ca.crt')
            ...
            self.logger.error("[CouchDBStore] Exception occured in get_subcategories_for_category: " + str(e) + ". Returning empty StoreResponse")
            response = self._create_store_response(400, str(e), {})
