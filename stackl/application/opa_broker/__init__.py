...
class OPABroker():
  ...
  # create input to hand to OPA
        ...
        try:
            response = requests.post(self.opa_host + "/v1/data/" + policy_package + "/" + policy_rule, data=json.dumps(input_dict))
    ...
    def convert_sat_to_opa_data(self, sat_doc: StackApplicationTemplate):
        ...
        sat_services = sat_doc.services
        services_as_data = []
        sat_as_opa_data = { "services": services_as_data}
        for service in sat_services:
            service_doc = document_manager.get_service(service)
            service_data = {}
