...
class ItemHandler(Handler):
    ...
    def process(self, item):
        logger.debug("[ItemHandler] process. Item to process: {0}".format(str(item)))
        item = item["doc"]["name"]
        action = item["action"]
    ...
    def handle(self, item):
        logger.debug("[ItemHandler] handle. Item to handle: {0}".format(str(item)))
        try:
            action = item["action"]
            logger.debug(
    ...
    def load_item_variables_delete(self, item):
        logger.debug("[ItemHandler] load_item_variables_delete. For item: {0}".format(str(item)))

        hierarchy = self.item_manager.get_item_hierarchy()
        merged_doc = {}
        for element in hierarchy:
            logger.debug("[ItemHandler] element: " + str(element))
            if element[0] == "common":
                doc_name = element[1]
            else:
    ...
    def delete_item_stack_instance(self, item, stack_instances_names):
        logger.debug("[ItemHandler] delete_item_stack_instance. deleting " + item)
        self.document_manager.remove_document(
            document_name=item, subcategory="item"
        )
        logger.debug("[ItemHandler] stack_instances " + str(stack_instances_names))
    ...
    def invoke_websocket(self, item):
        ...
        logger.debug("[ItemHandler] invoke_websocket. extra_vars for websocket: {0}".format(extra_vars))
        # logger.debug("[ItemHandler] change_obj[websocket]: "+ str(change_obj['websocket']))
    ...
    def flag_item_request_state(self, item_to_be_flagged, parent_item_names, action, parent_item_view_category="types",
        ...
        if parent_item_names is None:
            logger.debug("[ItemHandler] No parent_item_names. Returning.")
            return
        else:
            logger.debug("[ItemHandler] parent_item_names '{}'. Starting loop.".format(parent_item_names))
        for parent_item_name in parent_item_names:
            logger.debug("[ItemHandler] Current parent resource to modify '{}'".format(parent_item_name))
            logger.debug("[ItemHandler] Trying to modify.... " + parent_item_name)
            tries = 100
            stop = False
            while stop == False and tries > 0:
                ...
                logger.info("[ItemHandler] Lookup the stack instance document")
                ...
                logger.debug("[ItemHandler] Item: " + str(item))
                # Deprecated. See old code and redo if still needed
                # if item is not None:
                #     item_found = False
                #     for service in item["services"]:
                #         logger.debug("[ItemHandler] Checking if service '{0}' is item_to_be_flagged '{1}'".format(service, item_to_be_flagged))
                #         if item_to_be_flagged == service:
                #             item_found = True
                #             #DEPRECATE
                #     #         logger.debug("[ItemHandler] Adding state '{0}' to resource entry '{1}' for instance '{2}'".format(
                ...
                #     #         logger.debug("[ItemHandler] Writing status flag status code: '{}'".format(status_code))
                #     #         if tries <= 0 or status_code != 409:
                #     #             logger.debug("[ItemHandler] Stopping. Tries <= 0 or status_code not 409")
                #     #             stop = True
                #     #         else:
                #     #             tries = tries - 1
                #     #             logger.debug("[ItemHandler] Tries left: "+ str(tries))
                #     #             time.sleep(randint(0,5))
                #     # if item_found == False:
                #     #     logger.debug("[ItemHandler] Stop try to flag action '{0}' for resource '{1}'. Not found in resource list '{2}'".format(
                #     #                         action, resource_to_be_flagged, parent_resource_view_type))
                #     #     stop = True
                #     # item_found = False
                # else:
                #     #tries = tries - 1
                #     logger.debug("[ItemHandler] item not found. Breaking loop and continuing with other instances...")
                #     time.sleep(2)
                #     break

    def get_automation_invocation_enabled(self, item_name):
        logger.info("[ItemHandler] get_automation_invocation_enabled. Item_name '{0}'".format(str(item_name)))
        ...
        if automation_invocation_enabled == {}:
            ...
            logger.info("[ItemHandler] automation_invocation_enabled was bool... returning "
                        + str(automation_invocation_enabled)
                        )
            return automation_invocation_enabled
        elif str(automation_invocation_enabled).lower() == "true":
            logger.info("[ItemHandler] Automation_invocation_enabled was 'True'... returning True'")
            return True
        elif str(automation_invocation_enabled).lower() == "false":
            logger.info("[ItemHandler] Automation_invocation_enabled was 'False'... returning False'")
            return False
        else:
            logger.info("[ItemHandler] Default case... automation_invocation_enabled returning True")
            return True
